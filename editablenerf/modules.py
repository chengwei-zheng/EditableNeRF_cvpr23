...
# ZCW add
@gin.configurable(denylist=['name'])
class HyperBScoeffMLP(nn.Module):
  """An MLP that defines a bendy slicing surface through hyper space."""
  output_channels: int = gin.REQUIRED
  ...
  use_residual: bool = False
  activation_type: str = 'sigmoid-none'
  hidden_activation: types.Activation = nn.sigmoid  # lambda x: x  nn.relu  ZCW add
  output_activation: types.Activation = lambda x: x

  @nn.compact
  def __call__(self, BS_coeff):
    if self.depth == 0 and self.width == 0:
      if self.output_channels != BS_coeff.shape[-1]:
        raise ValueError('HyperBScoeffMLP.output_channels != BS_coeff.shape[-1]')
      return BS_coeff
    ...
    mlp = MLP(depth=self.depth,
              ...
              output_activation=self.output_activation)  # ZCW add
    if self.use_residual:
      return mlp(BS_coeff) + BS_coeff
    else:
      return mlp(BS_coeff)
...
# ZCW add
@gin.configurable(denylist=['name'])
class MultiKPointsMLP(nn.Module):
  """An MLP that defines a bendy slicing surface through hyper space."""
  output_channels: int = gin.REQUIRED
  min_deg: int = 0
  max_deg: int = 4
  ...
  input_kp_deg: int = gin.REQUIRED
  use_bg: bool = gin.REQUIRED

  @nn.compact
  def __call__(self, points, k_points, alpha=None, return_weights=False):

    # --- MLP + softmax ---
    points_feat = model_utils.posenc(points, self.min_deg, self.max_deg, use_identity=True, alpha=alpha)
    if self.input_kp_deg >= 0:
      kp_feat = model_utils.posenc(k_points, 0, self.input_kp_deg, use_identity=True)
      inputs = jnp.concatenate([points_feat, kp_feat], axis=-1)
    else:
      inputs = points_feat
    ...
    mlp = MLP(depth=self.depth,
              ...
              output_init=self.output_init)
    outputs = mlp(inputs)
    weights = nn.softmax(outputs)
    if return_weights:
      return weights

    batch_shape = k_points.shape[0:-1]
    k_points = k_points.reshape((*batch_shape, self.output_channels, 3))
    weights = jnp.broadcast_to(weights[..., None], (*batch_shape, self.output_channels, 3))
    weight_kp = jnp.sum(weights * k_points, axis=-2)
