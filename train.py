...
def _log_to_tensorboard(writer: tensorboard.SummaryWriter,
  ...
  # ZCW add
  _log_scalar('loss/K_points', stats.get('K_points_loss'))
  _log_scalar('loss/flow_loss', stats.get('flow_loss'))
  _log_scalar('loss/depth_loss', stats.get('depth_loss'))
  _log_scalar('loss/weights_reg_loss', stats.get('weights_reg_loss'))
  _log_scalar('params/weights_reg', scalar_params.weights_reg_loss_weight)
...
def main(argv):
  ...
  devices = jax.local_devices()[0:4]
  # devices = [jax.local_devices()[0], jax.local_devices()[1], jax.local_devices()[6], jax.local_devices()[7]]
  print(devices)
  if train_config.batch_size % len(devices) != 0:
    raise ValueError('Batch size must be divisible by the number of devices.')
  ...
  datasource = exp_config.datasource_cls(
      ...
      k_point_num=dummy_model.k_point_num,  # ZCW add
  ...
  flow_iter = None
  cmr_const = None
  if train_config.use_flow_loss:
    flow, cmr_const = datasource.load_flow()
    cmr_const['coor'] = datasource.load_local_coor()
    flow_dataset = tf.data.Dataset.from_tensor_slices(flow)
    flow_iter = datasets.iterator_from_dataset(
        flow_dataset,
        batch_size=len(devices)*2,
        prefetch_size=3,
        devices=devices)
  ...
  # ZCW add
  # weights_reg_sched = schedules.from_config(train_config.weights_reg_schedule)
  ...
  scalar_params = training.ScalarParams(
      ...
      K_points_loss_weight=train_config.K_points_loss_weight,  # ZCW add
      flow_loss_weight=train_config.flow_loss_weight,  # ZCW add
      depth_loss_weight=train_config.depth_loss_weight,  # ZCW add
      weights_reg_loss_weight=float(0.0))  # weights_reg_sched(0))  # ZCW add
  ...
  # ZCW init
  if train_config.use_flow_loss:
    print('init hyper_embed by datasource')
    state_dict = flax.serialization.to_state_dict(state.optimizer)
    # hyper_embed = jax.device_put_replicated(datasource.kp_init, devices=devices)

    ori_hyper_embed = state_dict['target']['model']['hyper_embed']['embed']['embedding']
    ori_hyper_embed = np.asarray(ori_hyper_embed[0])
    # new_hyper_embed -= 0.025
    kp_size = datasource.kp_init.shape[-1]
    hyper_init = np.concatenate((datasource.kp_init, ori_hyper_embed[..., kp_size:]), axis=1)
    hyper_embed = jax.device_put_replicated(hyper_init, devices=devices)

    state_dict['target']['model']['hyper_embed']['embed']['embedding'] = hyper_embed
    state = state.replace(
        optimizer=flax.serialization.from_state_dict(state.optimizer, state_dict))
  ...
  if jax.process_index() == 0:
    ...
    # ZCW change, debug gin.markdown
    # summary_writer.text('gin/train', textdata=gin.config.markdownify_operative_config_str(config_str), step=0)
  ...
  train_step = functools.partial(
      ...
      use_K_points_loss=train_config.use_K_points_loss,  # ZCW add
      use_flow_loss=train_config.use_flow_loss,  # ZCW add
      use_weights_reg_loss=train_config.use_weights_reg_loss,  # ZCW add
      k_point_num=datasource.k_point_num,  # ZCW add
      cmr_const=cmr_const,  # ZCW add
  )
  ...
  for step, batch in zip(range(init_step, train_config.max_steps + 1),
    ...
    if flow_iter is not None:
      batch['flow_data'] = next(flow_iter)
    ...
    scalar_params = scalar_params.replace(
        ...
        #weights_reg_loss_weight=weights_reg_sched(step))  # ZCW add

    # ZCW add
    # if step == 50000:
    #   scalar_params = scalar_params.replace(weights_reg_loss_weight=float(train_config.weights_reg_loss_weight / 2))
    #   print('weights_reg_loss_weight')
    # if step == 100000:
    #   scalar_params = scalar_params.replace(weights_reg_loss_weight=float(train_config.weights_reg_loss_weight))
    #   print('weights_reg_loss_weight')
    # if step == 150000:
    #   scalar_params = scalar_params.replace(weights_reg_loss_weight=float(0))
    #   print('weights_reg_loss_weight')
    ...
    if step % train_config.print_every == 0 and jax.process_index() == 0:
      logging.info('step=%d, nerf_alpha=%.04f, warp_alpha=%.04f, %s', step,  # weights_reg==%f,
                   nerf_alpha_sched(step),
                   warp_alpha_sched(step),
                   #weights_reg_sched(step),
    ...
    if step % train_config.save_every == 0 and jax.process_index() == 0:
      training.save_checkpoint(checkpoint_dir, state, keep=50) # ZCW change 2
