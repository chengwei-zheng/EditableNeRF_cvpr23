...
class NerfModel(nn.Module):
  ...
  # ZCW add
  # hyper_use_BS_coeff: bool = False
  load_kp_init_file: str = ''
  kp_dimension: int = 0
  k_point_num: int = 0
  multi_k_points_mlp_cls: Callable[..., nn.Module] = modules.MultiKPointsMLP
  # hyper_BS_coeff_mlp_cls: Callable[..., nn.Module] = modules.HyperBScoeffMLP
  ...
  # ZCW add
  # def hyper_from_BS_coeff(self, metadata):
  #   return self.hyper_BS_coeff_mlp(metadata)

  # ZCW add
  def get_multi_weights(self, points, hyper_embed):
    return self.multi_k_points_mlp(points, hyper_embed, return_weights=True)
  ...
  def setup(self):
    ...
    if self.hyper_slice_method == 'axis_aligned_plane':
      ...
      # ZCW add
      # if self.hyper_use_BS_coeff:
      #   self.hyper_BS_coeff_mlp = self.hyper_BS_coeff_mlp_cls()
      if self.k_point_num >= 2:
        self.multi_k_points_mlp = self.multi_k_points_mlp_cls()
  ...
  def map_spatial_points(self, points, warp_embed, extra_params, use_warp=True,
                         return_warp_jacobian=False, repose=None):
    warp_jacobian = None
    if self.use_warp and use_warp:
      # warp_fn = jax.vmap(jax.vmap(self.warp_field, in_axes=(0, 0, None, None)),
      #                    in_axes=(0, 0, None, None))
      # ZCW change
      warp_fn = jax.vmap(jax.vmap(self.warp_field, in_axes=(0, 0, None, None, 0)),
  ...
  def map_hyper_points(self, points, hyper_embed, extra_params,
    ...
    if hyper_point_override is not None:
      ...
      # ZCW change
      # if self.hyper_use_BS_coeff:
      #   hyper_points = self.hyper_BS_coeff_mlp(hyper_embed)
      if self.k_point_num >= 2:
        hyper_points = self.multi_k_points_mlp(points, hyper_embed)
      else:
        hyper_points = hyper_embed
    elif self.hyper_slice_method == 'bendy_sheet':
  ...
  def map_points(self, points, warp_embed, hyper_embed, extra_params,
                 ...
                 hyper_point_override=None, repose=None):
    ...
    spatial_points, warp_jacobian = self.map_spatial_points(
        ...
        return_warp_jacobian=return_warp_jacobian, repose=repose)

    # ZCW add
    if self.hyper_slice_method == 'axis_aligned_plane' and self.k_point_num >= 2:
      hyper_points = self.map_hyper_points(
            spatial_points, hyper_embed, extra_params,
  ...
  def render_samples(self,
    ...
    if self.has_hyper_embed:
      ...
      # ZCW add
      # elif self.hyper_use_BS_coeff:
      #   hyper_embed = metadata['BS_coeff']
      else:
    ...
    # ZCW add
    repose = metadata.get('repose')
    if repose is not None:
      repose = jnp.broadcast_to(
          repose[:, jnp.newaxis, :],
          shape=(*batch_shape, repose.shape[-1]))
    ...
    med_warped_points = jnp.take_along_axis(
        # Unsqueeze axes: sample axis, coords.
        warped_points, depth_indices[..., None, None], axis=-2)  # ZCW change warped_points / points
    out['med_warped_points'] = med_warped_points
    med_points = jnp.take_along_axis(
        points, depth_indices[..., None, None], axis=-2)  # ZCW change warped_points / points
  ...
  def __call__(
      ...
      metadata_encoded=False,
      use_warp=True,
      return_points=False, # ZCW change, ori: False
...
def construct_nerf(key, batch_size: int, embeddings_dict: Dict[str, int],
                   near: float, far: float):   # kp_dimension: int
  ...
  init_rays_dict = {
      ...
      'metadata': {
          ...
          # 'kp_dimension': jnp.ones((batch_size, kp_dimension), jnp.float32), # ZCW add  51  34 * 3
      }
  }
  extra_params = {
      'nerf_alpha': 0.0,
