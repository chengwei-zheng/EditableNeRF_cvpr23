  ...
  if save_dir:
    ...
    if tag == 'train' or tag == 'val':
      ...
      if 0:
        ...
        med_points = model_out['med_points'].squeeze()#[..., 0:3]  # [..., 3:]
        np.save(str(full_dir / f'med_points_{item_id}.npy'), med_points)

        med_warped_points = model_out['med_warped_points'].squeeze()#[..., 0:3]  # [..., 3:]
        np.save(str(full_dir / f'med_warped_points_{item_id}.npy'), med_warped_points)
...
def process_iterator(tag: str,
  ...
  for i, (item_id, batch) in enumerate(zip(item_ids, iterator)):
    ...
    if tag == 'test':
      # ZCW change
      # if datasource.use_BS_coeff:
      #   ValueError('BS coeff not supported in test now.')
      batch['metadata'] = sample_random_metadata(datasource, batch, int(item_id) + 1)  # step)

    # ZCW change
    batch['metadata'] = evaluation.encode_metadata(
        model, jax_utils.unreplicate(params), batch['metadata'], item_id)  # batch['metadata'], i
...
def main(argv):
  ...
  devices_to_use = jax.local_devices()[0:4]
  # devices_to_use = [jax.local_devices()[0], jax.local_devices()[1], jax.local_devices()[6], jax.local_devices()[7]]
  print(devices_to_use)
  n_devices = len(
  ...
  datasource = exp_config.datasource_cls(
      ...
      camera_mode=eval_config.camera_mode)  # ZCW add
