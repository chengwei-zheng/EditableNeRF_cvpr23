...
class ScalarParams:
  ...
  # ZCW add
  K_points_loss_weight: float = 0.0
  flow_loss_weight: float = 0.0
  depth_loss_weight: float = 0.0
  weights_reg_loss_weight: float = 0.0
...
@functools.partial(jax.jit,
                   ...
                   static_argnames=('disable_hyper_grads',
                                    ...
                                    'use_warp_reg_loss',
                                    'use_hyper_reg_loss',
                                    'use_K_points_loss',  # ZCW add
                                    'use_flow_loss',  # ZCW add
                                    'use_weights_reg_loss',  # ZCW add
                                    'k_point_num'))  # ZCW add
def train_step(model: models.NerfModel,
               ...
               use_K_points_loss: bool = False,  # ZCW add
               use_flow_loss: bool = False,  # ZCW add
               use_weights_reg_loss: bool = False,  # ZCW add
               k_point_num: int = 1,  # ZCW add
               cmr_const: Dict[str, Any] = None):  # ZCW add
  ...
  if cmr_const:
    focal_length = cmr_const['focal_length']
    radial_distortion = cmr_const['radial_distortion']
    tangential_distortion = cmr_const['tangential_distortion']
    skew = cmr_const['skew']
    principal_point_x = cmr_const['principal_point'][0]
    principal_point_y = cmr_const['principal_point'][1]
    pixel_aspect_ratio = cmr_const['pixel_aspect_ratio']

    coor_scale = cmr_const['coor']['scale']
    coor_bias = cmr_const['coor']['bias']
  ...
  def _compute_loss_and_stats(
    ...
    if use_warp_reg_loss:
      ...
      # stats['loss/warp_reg2'] = 0.5 * warp_reg_residual.mean() / scalar_params.warp_reg_loss_scale
      warp_reg_loss = 0.5 * warp_reg_residual.mean() / scalar_params.warp_reg_loss_scale
  ...
  def cmr_project(point, orientation, position):
    if not cmr_const:
      raise ValueError('cmr_const is None.')

    translated_points = point - position
    local_points = (jnp.matmul(orientation, translated_points.T)).T

    # depth = jnp.linalg.norm(local_points, axis=-1)
    depth = jnp.linalg.norm(local_points)

    # Get normalized local pixel positions.
    # z = jnp.maximum(local_points[..., 2], 1e-8)
    ...
    distortion = 1.0 + r2 * (
        radial_distortion[0] + r2 *
        (radial_distortion[1] + radial_distortion[2] * r2))

    # Apply tangential distortion.
    x_times_y = x * y
    x = (
        x * distortion + 2.0 * tangential_distortion[0] * x_times_y +
        tangential_distortion[1] * (r2 + 2.0 * x**2))
    y = (
        y * distortion + 2.0 * tangential_distortion[1] * x_times_y +
        tangential_distortion[0] * (r2 + 2.0 * y**2))

    # Map the distorted ray to the image plane and return the depth.
    pixel_x = focal_length * x + skew * y + principal_point_x
    pixel_y = (focal_length * pixel_aspect_ratio * y
               + principal_point_y)

    # pixel_x = focal_length * x + 160
    # pixel_y = focal_length * y + 90
    pixel = jnp.stack([pixel_x, pixel_y], axis=-1)
    return pixel, depth
  ...
  def _loss_fn(params):
    ...
    # ZCW add
    if use_K_points_loss:
      points = model.apply({'params': params['model']}, batch['metadata'], method=model.encode_hyper_embed)
      points_loss = ((points - batch['metadata']['kp_init'])**2).mean()
      losses['K_points'] = scalar_params.K_points_loss_weight * points_loss
      stats['K_points_loss'] = points_loss

    # ZCW add
    if use_weights_reg_loss:
      med_points = lax.stop_gradient(ret['coarse']['med_points'])   # NOT WARPED

      # hyper_embed_wr = batch['metadata'][model.hyper_embed_key]
      hyper_embed_wr = model.apply({'params': params['model']},
                                   {model.hyper_embed_key: batch['metadata'][model.hyper_embed_key]},
                                   method=model.encode_hyper_embed)
      hyper_embed_wr = lax.stop_gradient(hyper_embed_wr)
      hyper_embed_wr = jnp.broadcast_to(
          hyper_embed_wr[:, jnp.newaxis, :],
          shape=(*med_points.shape[:-1], hyper_embed_wr.shape[-1]))

      weights = model.apply(
        {'params': params['model']}, med_points, hyper_embed_wr, method=model.get_multi_weights)
      weights_reg_loss = -((weights - 0.5)**2).mean()
      losses['weights_reg'] = scalar_params.weights_reg_loss_weight * weights_reg_loss
      stats['weights_reg_loss'] = weights_reg_loss

    # ZCW add
    if use_flow_loss:
      k_points_world = model.apply({'params': params['model']},
                            batch['flow_data']['metadata'], method=model.encode_hyper_embed)

      # (2 frames[t & t+1], key point number, 3d position)
      k_points_world = k_points_world.reshape((2, -1, 3))

      k_points_world = k_points_world.at[:, 0:k_point_num, :].add(-coor_bias)
      k_points_world = k_points_world.at[:, 0:k_point_num, :].multiply(1.0 / coor_scale)

      for kp in range(k_point_num):
        pix0, depth0 = cmr_project(k_points_world[0, kp, :],
                                   batch['flow_data']['cmr_orient'][0], batch['flow_data']['cmr_pos'][0])
        pix1, depth1 = cmr_project(k_points_world[1, kp, :],
                                   batch['flow_data']['cmr_orient'][1], batch['flow_data']['cmr_pos'][1])

        r0 = pix0[1].astype(int)
        c0 = pix0[0].astype(int)
        r1 = pix1[1].astype(int)
        c1 = pix1[0].astype(int)

        kp_str = '_' + str(kp)

        diff = pix1 - pix0
        flow_error = ((diff - batch['flow_data']['flow'][0, r0, c0])**2).mean()
        losses['flow' + kp_str] = scalar_params.flow_loss_weight * flow_error
        stats['flow_loss' + kp_str] = flow_error
        stats['flow_pix0_r' + kp_str] = pix0[1]
        stats['flow_pix0_c' + kp_str] = pix0[0]
        # stats['flow_pix1_r'] = pix1[1]
        # stats['flow_pix1_c'] = pix1[0]

        depth_error0 = depth0 - batch['flow_data']['depth'][0, r0, c0]
        depth_error1 = depth1 - batch['flow_data']['depth'][1, r1, c1]
        losses['depth' + kp_str] = scalar_params.depth_loss_weight * (depth_error0**2 + depth_error1**2) / 2
        stats['depth_loss' + kp_str] = (depth_error0**2 + depth_error1**2) / 2  # mean
        # stats['depth0'] = depth0
        # stats['depth_in0'] = batch['flow_data']['depth'][0, r, c]
