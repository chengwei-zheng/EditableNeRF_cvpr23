...
from editablenerf import tf_camera as tfcam
...
class DataSource(abc.ABC):
  ...
  def __init__(self,
               ...
               use_camera_id=False,
               use_warp_id=False,
               use_depth=False,   # ZCW change
               use_relative_depth=False,
               use_time=False,
               load_kp_init_file='',  # ZCW add
               k_point_num=1,  # ZCW add
               kp_dimension=0,  # ZCW add
               # BS_coeff=None,  # ZCW add
               camera_mode=0,  # ZCW add
               random_seed=0,
               train_stride=1,
               val_stride=1,
               preload=True,
    ...
    self.load_kp_init_file = load_kp_init_file  # ZCW add
    self.k_point_num = k_point_num  # ZCW add
    self.kp_dimension = kp_dimension  # ZCW add
    # self.BS_coeff = BS_coeff  # ZCW add
    self.camera_mode = camera_mode  # ZCW add
  ...
  def get_item(self, item_id, scale_factor=1.0):
    ...
    #   data['metadata']['kp_init'] = np.atleast_1d(self.get_kp_init(item_id))
    ...
    if self.use_depth:
      ...
      if depth is not None:
        ...
        depth = image_utils.downsample_image_NEAREST(depth, 4) # ZCW add
